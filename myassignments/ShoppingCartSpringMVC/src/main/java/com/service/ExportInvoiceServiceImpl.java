...
public class ExportInvoiceServiceImpl implements ExportInvoiceService{
	...
	public void createXML(TotalDetails totalDetails) {
		int invno = totalDetails.getInvid();
		System.out.println("Creating XML");
		StringBuilder sb=new StringBuilder();
		LocalDate localdate=totalDetails.getInvoiceDate();
		String date = localdate.toString();
		HashMap<ItemMaster,Integer> itemtransactiondetails = totalDetails.getItemList();
		CustomerMaster customerobj = totalDetails.getCustomerMaster();
		int custno=customerobj.getCustomerid();
		...
		sb.append("<customeremail>"+customerobj.getUser().getEmail()+"</customeremail>");
		sb.append("<customerphone>"+customerobj.getCustomerPhone()+"</customerphone>");
		sb.append("</customer>");
		sb.append("<items>");
		double sum=0;
		for (Map.Entry<ItemMaster, Integer> i : itemtransactiondetails.entrySet()) {
			sb.append("<item>"); 
	        sb.append("<itemno>"+i.getKey().getItemid()+"</itemno>");
	        sb.append("<itemdescription>"+i.getKey().getItemName()+"</itemdescription>");
	        sb.append("<itemunit>"+i.getKey().getItemUnit()+"</itemunit>");
	        sb.append("<itemprice>"+i.getKey().getItemPrice()+"</itemprice>");
	        sb.append("<itemqty>"+i.getValue()+"</itemqty>");
	        double total = i.getKey().getItemPrice()*i.getValue();
	        sum+=total;
		...
		sb.append("</invoice>");
	    try {
			File output = new File("D:/EYIntern/JAVA/workspace/ShoppingCartSpringMVC/results/"+invno+".xml");
	...
	public void XMLToPDF(int invno) {
		try {
			...
			PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("D:/EYIntern/JAVA/workspace/ShoppingCartSpringMVC/results/"+invno+".pdf"));
		    document.open();
			...
			sp.parse("D:/EYIntern/JAVA/workspace/ShoppingCartSpringMVC/results/"+invno+".xml", handle);
	...
	public void XMLToExcel(int invno) {
		try {
			...
			OutputStream fileOut = new FileOutputStream("D:/EYIntern/JAVA/workspace/ShoppingCartSpringMVC/results/"+invno+".xls");   
	...
	public void XMLToSMS(int invno) {
		try {
			  ...
			  String AUTH_TOKEN = "91856ad020169166b261eca2b013b010";  
	...
	public TotalDetails getTotalDetails(int invno) {
		TotalDetails totalDetails = invoiceMasterDAO.getTotalDetails(invno);
		return totalDetails;
	}
